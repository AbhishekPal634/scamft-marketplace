...
const mapDbPurchaseToFrontend = async (
  purchase: any,
  purchaseItems: any[]
): Promise<Purchase> => {
...
export interface NFTStore {
  ...
  completePurchase: (purchaseId: string, userId: string) => Promise<boolean>; // Add new function
}
...
export const useNFTStore = create<NFTStore>((set, get) => ({
  ...
  getUserPurchases: async (userId: string) => {
    try {
      ...
      for (const purchase of purchasesData) {
        ...
        const mappedPurchase = await mapDbPurchaseToFrontend(
          purchase,
          itemsData
        );
  ...
  filterNFTs: (filters: NFTFilters) => {
    ...
    if (filters.minPrice !== undefined) {
      filteredNfts = filteredNfts.filter(
        (nft) => nft.price >= filters.minPrice!
      );
    }

    if (filters.maxPrice !== undefined) {
      filteredNfts = filteredNfts.filter(
        (nft) => nft.price <= filters.maxPrice!
      );
    }
  ...
  completePurchase: async (purchaseId: string, userId: string) => {
    try {
      // Call the database function to complete the purchase
      ...
      // Refresh the NFT list to reflect the changes
      await get().fetchMarketplaceNFTs();
      await get().fetchNFTs();
